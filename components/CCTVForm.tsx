...
interface CCTVFormProps {
    initialData?: Partial<CCTVFormData>
    isEdit?: boolean
    cctvId?: string
    onSuccess?: () => void
}

export default function CCTVForm({ initialData, isEdit = false, cctvId, onSuccess }: CCTVFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit && cctvId) {
                await axios.put(`/api/cctv/${cctvId}`, submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                ...
                await axios.post('/api/cctv', submitData)
                await Swal.fire({
                    title: 'Berhasil!',
            ...
            await Swal.fire({
                title: 'Gagal!',
                text: 'Gagal menyimpan data CCTV. Periksa kembali isian form.',
                icon: 'error',
                confirmButtonText: 'OK',
                buttonsStyling: false,
                customClass: {
    ...
    return (
        <form onSubmit={handleSubmit} className="premium-card p-6 sm:p-8 lg:p-10 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Storage */}
                <div className="space-y-3">
                    <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest pl-1">Storage</label>
                    <input 
                        type="text" 
                    ...
                    <div className="space-y-4">
                        {preview ? (
                            <div className="relative">
                                <img 
                                    src={preview.startsWith('/') || preview.startsWith('http') ? preview : ''} 
                                    alt="Preview" 
            ...
            <div className="flex items-center justify-end gap-4 pt-4">
                ...
                >
                    {loading ? 'Menyimpan...' : isEdit ? 'Update CCTV' : 'Simpan CCTV'}
                </button>
            </div>
        </form>
    )
