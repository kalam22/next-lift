...
export default function PrinterForm({ initialData, isEdit = false, printerId, onSuccess }: PrinterFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const submitData = {
                ...
                jumlah: formData.jumlah ? parseInt(String(formData.jumlah)) : 0,
                ...
                kerusakan: formData.kerusakan.trim() || null,
                keterangan: formData.keterangan.trim() || null,
                foto: formData.foto || null,
            }

            if (isEdit && printerId) {
                await axios.put(`/api/printer/${printerId}`, submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                ...
                await axios.post('/api/printer', submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                    text: 'Data Printer berhasil ditambahkan',
                    icon: 'success',
                    timer: 1500,
                    showConfirmButton: false,
                    buttonsStyling: false,
            ...
            await Swal.fire({
                title: 'Gagal!',
                text: error.response?.data?.message || 'Gagal menyimpan data Printer',
                icon: 'error',
                confirmButtonText: 'OK',
                buttonsStyling: false,
                customClass: {
    ...
    return (
        <form onSubmit={handleSubmit} className="premium-card p-6 sm:p-8 lg:p-10 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Jumlah */}
                <div className="space-y-3">
                    <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest pl-1">Jumlah <span className="text-red-500">*</span></label>
                    <input 
                        type="number" 
                        required 
                        min="1"
            ...
            <div className="flex items-center justify-end gap-4 pt-4">
                ...
                >
                    {loading ? 'Menyimpan...' : isEdit ? 'Update Printer' : 'Simpan Printer'}
                </button>
            </div>
        </form>
    )
