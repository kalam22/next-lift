...
export default function MonitorForm({ initialData, isEdit = false, monitorId, onSuccess }: MonitorFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit && monitorId) {
                await axios.put(`/api/monitor/${monitorId}`, submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                    text: 'Data monitor berhasil diperbarui',
                    icon: 'success',
                    timer: 1500,
                ...
                await axios.post('/api/monitor', submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                    text: 'Data monitor berhasil ditambahkan',
                    icon: 'success',
                    timer: 1500,
                    showConfirmButton: false,
                    buttonsStyling: false,
            ...
            await Swal.fire({
                title: 'Gagal!',
                text: error.response?.data?.message || 'Gagal menyimpan data monitor',
                icon: 'error',
                confirmButtonText: 'OK',
                buttonsStyling: false,
                customClass: {
    ...
    return (
        <form onSubmit={handleSubmit} className="premium-card p-6 sm:p-8 lg:p-10 space-y-6">
            ...
            <div className="flex items-center justify-end gap-4 pt-4">
                ...
                >
                    {loading ? 'Menyimpan...' : isEdit ? 'Update Monitor' : 'Simpan Monitor'}
                </button>
            </div>
        </form>
    )
