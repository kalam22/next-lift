...
export default function StorageForm({ initialData, isEdit = false, storageId, onSuccess }: StorageFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit && storageId) {
                await axios.put(`/api/storage/${storageId}`, submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                    text: 'Data Storage berhasil diperbarui',
                    icon: 'success',
                    timer: 1500,
                ...
                await axios.post('/api/storage', submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                    text: 'Data Storage berhasil ditambahkan',
                    icon: 'success',
                    timer: 1500,
                    showConfirmButton: false,
                    buttonsStyling: false,
            ...
            await Swal.fire({
                title: 'Gagal!',
                text: error.response?.data?.message || 'Gagal menyimpan data Storage',
                icon: 'error',
                confirmButtonText: 'OK',
                buttonsStyling: false,
                customClass: {
    ...
    return (
        <form onSubmit={handleSubmit} className="premium-card p-6 sm:p-8 lg:p-10 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                    ...
                    <input 
                        ...
                        onBlur={(e) => setFormData({ ...formData, storage: appendGB(e.target.value) })}
                        placeholder="500..." 
                        className="w-full px-4 py-3 bg-gray-50 dark:bg-[#0f172a] border border-[#f1f5f9] dark:border-[#334155] rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary/50 transition-all outline-none font-bold text-[#0f172a] dark:text-white" 
                    />
                </div>
            ...
            <div className="flex items-center justify-end gap-4 pt-4">
                ...
                >
                    {loading ? 'Menyimpan...' : isEdit ? 'Update Storage' : 'Simpan Storage'}
                </button>
            </div>
        </form>
    )
