...
interface ToolsJaringanFormData {
    brand: string
...
interface ToolsJaringanFormProps {
    initialData?: Partial<ToolsJaringanFormData>
    isEdit?: boolean
    toolsJaringanId?: string
    onSuccess?: () => void
}

export default function ToolsJaringanForm({ initialData, isEdit = false, toolsJaringanId, onSuccess }: ToolsJaringanFormProps) {
    ...
    const [formData, setFormData] = useState<ToolsJaringanFormData>({
        brand: '',
        jumlahOrderan: '',
        diperuntukan: '',
        site: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit && toolsJaringanId) {
                await axios.put(`/api/tools-jaringan/${toolsJaringanId}`, submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                    text: 'Data toolsJaringan berhasil diperbarui',
                    icon: 'success',
                    timer: 1500,
                ...
                await axios.post('/api/tools-jaringan', submitData)
                await Swal.fire({
                    title: 'Berhasil!',
                    text: 'Data Tools Jaringan berhasil ditambahkan',
                    icon: 'success',
                    timer: 1500,
                    showConfirmButton: false,
                    buttonsStyling: false,
            ...
            if (onSuccess) {
                ...
                router.push('/tools-jaringan')
            }
        } catch (error: any) {
            logger.error('Error submitting form:', error)
            await Swal.fire({
                title: 'Gagal!',
                text: error.response?.data?.message || 'Gagal menyimpan data Tools Jaringan',
                icon: 'error',
                confirmButtonText: 'OK',
                buttonsStyling: false,
                customClass: {
    ...
    return (
        <form onSubmit={handleSubmit} className="premium-card p-6 sm:p-8 lg:p-10 space-y-6">
            ...
            <div className="flex items-center justify-end gap-4 pt-4">
                ...
                >
                    {loading ? 'Menyimpan...' : isEdit ? 'Update ToolsJaringan' : 'Simpan ToolsJaringan'}
                </button>
            </div>
        </form>
    )
