...
export default function ViewCCTV() {
    ...
    const { canEdit } = usePermissions('cctv')
    ...
    const fetchCCTV = useCallback(async () => {
        try {
            ...
            setCctv(cctvRes.data); setNav(navRes.data)
        } catch (error: unknown) {
            logger.error('Error fetching cctv:', error)
            await Swal.fire({ title: 'Gagal!', text: 'Gagal memuat data CCTV', icon: 'error', confirmButtonText: 'OK', buttonsStyling: false, customClass: { popup: '!rounded-2xl', title: '!font-bold', confirmButton: 'swal2-confirm' } })
    ...
    const statusColor = () => {
        const s = cctv.statusBarang?.toUpperCase()
        ...
        if (s === 'SERVICE' || cctv.statusBarang?.toLowerCase().includes('rusak')) return 'bg-red-50 dark:bg-red-500/10 text-red-600 border-red-100 dark:border-red-900/40'
        ...
        <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <Link href="/cctv" className="p-2.5 text-gray-400 hover:text-blue-600 hover:bg-white dark:hover:bg-[#1e293b] rounded-2xl border border-transparent hover:border-[#f1f5f9] dark:hover:border-[#334155] transition-all shadow-sm">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                    </Link>
                    <div>
                        <h1 className="text-2xl font-black text-[#0f172a] dark:text-white tracking-tight uppercase">{cctv.brand}</h1>
                        <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-xs font-bold text-blue-600 uppercase tracking-widest">{cctv.diperuntukan}</span>
                ...
                <div className="flex items-center gap-2">
                    <button onClick={() => nav.prevId && router.push(`/cctv/${nav.prevId}/view`)} disabled={!nav.prevId} className="p-2 bg-white dark:bg-[#1e293b] border border-[#f1f5f9] dark:border-[#334155] rounded-xl text-gray-400 hover:text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
                    </button>
                    <button onClick={() => nav.nextId && router.push(`/cctv/${nav.nextId}/view`)} disabled={!nav.nextId} className="p-2 bg-white dark:bg-[#1e293b] border border-[#f1f5f9] dark:border-[#334155] rounded-xl text-gray-400 hover:text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                    </button>
                    <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${statusColor()}`}>{cctv.statusBarang}</span>
                    {canEdit && <Link href={`/cctv/${cctv.id}/edit`} className="px-4 py-2 bg-white dark:bg-[#1e293b] border border-[#f1f5f9] dark:border-[#334155] rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-600 dark:text-gray-300 hover:bg-gray-50 transition-all shadow-sm">Edit</Link>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="space-y-6">
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm overflow-hidden p-3">
                        <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-50 dark:bg-[#0f172a]">
                            {cctv.foto ? <Image src={cctv.foto} alt={cctv.brand} fill className="object-cover" priority unoptimized={cctv.foto?.startsWith('/')} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} /> : (
                    ...
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-6">
                        ...
                        <InfoRow label="Site" value={cctv.site} highlight />
                        <InfoRow label="Departemen" value={cctv.departemen} />
                        <InfoRow label="Nomor PO" value={cctv.nomorPO ? `#${cctv.nomorPO}` : '-'} />
                        <InfoRow label="Tgl Masuk" value={cctv.tanggalMasuk ? formatDateWITA(cctv.tanggalMasuk) : '-'} />
                        <InfoRow label="Tgl Kirim" value={cctv.tanggalKirim ? formatDateWITA(cctv.tanggalKirim) : '-'} />
                        <InfoRow label="Surat Jalan" value={cctv.nomorSuratJalan} />
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-6">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">{cctv.keterangan || 'Belum ada keterangan.'}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-center">
                <div className="w-full max-w-xl">
                    <ActivityLogPanel entityType="cctv" entityId={cctv.id} />
                </div>
            </div>
        </div>
    )
