          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 bg-gray-100/50 dark:bg-white/5 p-1 rounded-xl">
              <button
                ...
                className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${statusFilter === 'all' ? 'bg-white dark:bg-[#0f172a] text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200'}`}
              >
                All
              </button>
              <button
                onClick={() => setStatusFilter('active')}
                className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${statusFilter === 'active' ? 'bg-white dark:bg-[#0f172a] text-primary shadow-sm' : 'text-gray-500 hover:text-primary'}`}
              >
                Active
              </button>
              <button
                onClick={() => setStatusFilter('expiring_soon')}
                className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${statusFilter === 'expiring_soon' ? 'bg-white dark:bg-[#0f172a] text-amber-500 shadow-sm' : 'text-gray-500 hover:text-amber-500'}`}
              >
                Will Expire
              </button>
              <button
                onClick={() => setStatusFilter('expired')}
                className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${statusFilter === 'expired' ? 'bg-white dark:bg-[#0f172a] text-red-500 shadow-sm' : 'text-gray-500 hover:text-red-500'}`}
              >
                Expired
              </button>
            </div>
