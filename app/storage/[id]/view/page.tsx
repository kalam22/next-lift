...
export default function ViewStorage() {
    ...
    const { canEdit } = usePermissions('storage')
    const [storage, setStorage] = useState<Storage | null>(null)
    const [loading, setLoading] = useState(true)

    const fetchData = async () => {
        try {
            const res = await axios.get<Storage>(`/api/storage/${params.id}`)
            setStorage(res.data)
        } catch (error: unknown) {
            logger.error('Error fetching storage:', error)
            await Swal.fire({ title: 'Gagal!', text: 'Gagal memuat data Storage', icon: 'error', confirmButtonText: 'OK', buttonsStyling: false, customClass: { popup: '!rounded-2xl', title: '!font-bold', confirmButton: 'swal2-confirm' } })
    ...
    const statusColor = () => {
        const s = storage.statusBarang?.toUpperCase()
        ...
        if (s === 'SERVICE' || storage.statusBarang?.toLowerCase().includes('rusak')) return 'bg-red-50 dark:bg-red-500/10 text-red-600 border-red-100 dark:border-red-900/40'
        ...
        <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <Link href="/storage" className="p-2.5 text-gray-400 hover:text-blue-600 hover:bg-white dark:hover:bg-[#1e293b] rounded-2xl border border-transparent hover:border-[#f1f5f9] dark:hover:border-[#334155] transition-all shadow-sm">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                    </Link>
                    <div>
                        <h1 className="text-2xl font-black text-[#0f172a] dark:text-white tracking-tight uppercase">{storage.brand}</h1>
                        <span className="text-xs font-bold text-blue-600 uppercase tracking-widest">{storage.diperuntukan}</span>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${statusColor()}`}>{storage.statusBarang}</span>
                    {canEdit && <Link href={`/storage/${storage.id}/edit`} className="px-4 py-2 bg-white dark:bg-[#1e293b] border border-[#f1f5f9] dark:border-[#334155] rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-600 dark:text-gray-300 hover:bg-gray-50 transition-all shadow-sm">Edit</Link>}
                </div>
            </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm overflow-hidden p-3">
                        <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-50 dark:bg-[#0f172a]">
                            {storage.foto ? <Image src={storage.foto} alt={storage.brand} fill className="object-cover" priority unoptimized={storage.foto?.startsWith('/')} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} /> : (
                                <div className="w-full h-full flex flex-col items-center justify-center text-gray-300">
                                    <svg className="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4" /></svg>
                    ...
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-6">
                        ...
                        <InfoRow label="Site" value={storage.site} highlight />
                        <InfoRow label="Departemen" value={storage.departemen} />
                        <InfoRow label="Nomor PO" value={storage.nomorPO ? `#${storage.nomorPO}` : '-'} />
                        <InfoRow label="Tgl Masuk" value={storage.tanggalMasuk ? formatDateWITA(storage.tanggalMasuk) : '-'} />
                        <InfoRow label="Tgl Kirim" value={storage.tanggalKirim ? formatDateWITA(storage.tanggalKirim) : '-'} />
                        <InfoRow label="Surat Jalan" value={storage.nomorSuratJalan} />
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm overflow-hidden">
                        ...
                        <div className="p-6 grid grid-cols-2 gap-3">
                            ...
                            <SpecItem label="Status" value={storage.statusBarang} />
                            <SpecItem label="Kapasitas" value={storage.storage} />
                            <SpecItem label="Jumlah Orderan" value={storage.jumlahOrderan?.toString()} />
                            <SpecItem label="Diperuntukan" value={storage.diperuntukan} />
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-6">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">{storage.keterangan || 'Belum ada keterangan.'}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-center">
                <div className="w-full max-w-xl">
                    <ActivityLogPanel entityType="storage" entityId={storage.id} />
                </div>
            </div>
        </div>
    )
