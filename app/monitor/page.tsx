...
export default function MonitorPage() {
  ...
  // Setup data table hook - menggantikan semua state management
  const {
    data: monitors,
    loading,
    error,
    ...
    fetchData,
    colorTheme,
  } = useDataTable<Monitor>({
    apiEndpoint: '/api/monitor',
    entityName: 'Monitor',
    entityNamePlural: 'Monitor',
    colorTheme: 'blue',
  })

  // Setup Excel import hook
  ...
  const { handleExport } = useExcelExport({
    ...
    columns: [
      ...
      { header: 'DIPERUNTUKKAN KEPADA', key: 'diperuntukan', width: 25 },
      { header: 'SITE', key: 'site', width: 12 },
      { header: 'NO.PO', key: 'nomorPO', width: 15 },
      { header: 'NO.SURAT JALAN', key: 'nomorSuratJalan', width: 18 },
      { header: 'STATUS BARANG', key: 'statusBarang', width: 18 },
      { header: 'TANGGAL MASUK BARANG KE IT', key: 'tanggalMasuk', width: 28 },
      { header: 'TANGGAL KIRIM BARANG', key: 'tanggalKirim', width: 22 },
      { header: 'KETERANGAN', key: 'keterangan', width: 40 },
      { header: 'FOTO', key: 'foto', width: 30 },
    ],
    filename: 'Data_Monitor',
    worksheetName: 'Data Monitor',
    imageField: {
      fieldName: 'foto',
      columnIndex: 12,
    },
  ...
  // UI tetap sama, hanya menggunakan data dan handlers dari hooks
  return (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl sm:text-4xl font-black text-[#020617] dark:text-white tracking-tighter uppercase">
            Data <span className="text-primary">Monitor</span>
          </h1>
          <p className="text-xs sm:text-sm text-gray-400 font-bold uppercase tracking-[0.2em] opacity-60">Inventory / Monitor Units</p>
        </div>
        <div className="flex items-center justify-center sm:justify-start gap-3 flex-wrap">
          ...
          >
            ...
            <span className="uppercase tracking-widest">Register Monitor</span>
          </Link>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100/50 dark:divide-white/[0.02]">
              {loading ? (
                ...
                <tr>
                  <td colSpan={11} className="px-4 sm:px-8 py-24 sm:py-32 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-50">
                      ...
                      <span className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">No Monitor Registered</span>
                    </div>
                  </td>
                </tr>
              ) : (
                monitors.map((monitor: Monitor, index: number) => {
                  ...
                  return (
                    <tr key={monitor.id} className={`group hover:bg-primary/5/30 dark:hover:bg-primary/50/[0.02] transition-all duration-500 ease-out ${selectedItems.has(monitor.id) ? 'bg-primary/5/50 dark:bg-primary/50/10' : ''}`}>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <div className="flex items-center">
                          <input
                            type="checkbox"
                            checked={selectedItems.has(monitor.id)}
                            onChange={(e) => handleSelectItem(monitor.id, e.target.checked)}
                            className="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-primary focus:ring-2 cursor-pointer"
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <Link href={`/monitor/${monitor.id}/view`}>
                          <span className="text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight group-hover:text-primary transition-colors cursor-pointer hover:text-primary">{monitor.brand}</span>
                        </Link>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs font-black text-gray-700 dark:text-gray-300">{monitor.jumlahOrderan}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs sm:text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight underline decoration-primary/20 underline-offset-8 decoration-2">{monitor.diperuntukan}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                      ...
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs font-bold text-gray-600 dark:text-gray-300">#{monitor.nomorPO}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <div className={`inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-1.5 rounded-full ring-1 ring-inset ${
                          ...
                          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] whitespace-nowrap">{monitor.statusBarang}</span>
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        {monitor.foto ? (
                          <div className="flex items-center justify-center">
                            {imageErrors.has(monitor.id) ? (
                              ...
                              <Image
                                ...
                                onClick={() => {
                                  if (monitor.foto) {
                                    window.open(monitor.foto, '_blank')
                                  }
                                }}
                                loading="lazy"
                                placeholder="blur"
                                ...
                                onError={() => {
                                  setImageErrors(prev => new Set(prev).add(monitor.id))
                                }}
                              />
                            )}
                          </div>
