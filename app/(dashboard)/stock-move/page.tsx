...
import { handleError, extractErrorMessage } from '@/lib/stock-move/utils'
import { 
  FOCUS_DELAY_MS, 
  DEBOUNCE_DELAY_MS, 
  REFETCH_DELAY_MS,
...
function DynamicSelect({ value, onChange, apiEndpoint, placeholder = 'Pilih...', disabled = false }: {
  ...
  const fetchItems = useCallback(async () => {
    try { 
      const r = await axios.get<{ id: number; nama: string }[]>(apiEndpoint)
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    try {
      await axios.post(apiEndpoint, { nama: t })
      await fetchItems()
      onChange(t)
      setAddingNew(false)
      setNewName('')
...
function NamaBarangInput({ value, onChange, typeBarang }: { value: string; onChange: (v: string) => void; typeBarang: string }) {
    ...
    const fetchItemNames = async () => {
      try {
        const r = await axios.get<string[]>(
          `${API_ENDPOINTS.namaSuggestions}?typeBarang=${encodeURIComponent(typeBarang)}`
        )
        setSuggestions(r.data || [])
        // Auto-show dropdown if there are suggestions
        if ((r.data || []).length > 0) {
          setShowSuggestions(true)
        }
      } catch (error) {
        logger.error('Failed to fetch item names:', error)
        setSuggestions([])
        setShowSuggestions(false)
      }
    }
  ...
  const handleChange = (v: string) => {
    ...
    timerRef.current = setTimeout(async () => {
      try {
        const r = await axios.get<string[]>(
          `${API_ENDPOINTS.namaSuggestions}?q=${encodeURIComponent(v)}&typeBarang=${encodeURIComponent(typeBarang)}`
        )
        setSuggestions(r.data || [])
        setShowSuggestions((r.data || []).length > 0)
      } catch (error) {
...
function AddTransactionForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const validate = () => {
    ...
    if (!form.tanggal) e.tanggal = VALIDATION_MESSAGES.tanggalRequired
    if (!form.typeBarang.trim()) e.typeBarang = VALIDATION_MESSAGES.typeBarangRequired
    if (!form.namaBarang.trim()) e.namaBarang = VALIDATION_MESSAGES.namaBarangRequired
    if (!form.quality || Number(form.quality) < 1) e.quality = VALIDATION_MESSAGES.qualityMinimum
    setErrors(e)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await axios.post(API_ENDPOINTS.stockMove, {
        partType: form.partType,
        tanggal: form.tanggal,
        namaBarang: form.namaBarang.trim(),
        typeBarang: form.typeBarang.trim(),
        quality: Number(form.quality),
        vendorTujuan: form.vendorTujuan.trim() || null,
      ...
      const errorMessage = handleError(err, 'Failed to save transaction')
      Swal.fire({
        ...SWAL_ERROR_CONFIG,
        title: 'Gagal!',
        text: errorMessage || ERROR_MESSAGES.saveFailed,
      })
    } finally {
      setSubmitting(false)
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          {errors.tanggal && <p role="alert" aria-live="polite" className="text-[10px] text-red-500 font-bold">{errors.tanggal}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          {errors.typeBarang && <p role="alert" aria-live="polite" className="text-[10px] text-red-500 font-bold">{errors.typeBarang}</p>}
        </div>
        <div className="space-y-1.5">
          ...
          {errors.namaBarang && <p role="alert" aria-live="polite" className="text-[10px] text-red-500 font-bold">{errors.namaBarang}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          {errors.quality && <p role="alert" aria-live="polite" className="text-[10px] text-red-500 font-bold">{errors.quality}</p>}
        </div>
        <div className="space-y-1.5">
          {form.partType === 'MASUK' ? (
...
function EditRow({ transaction, onSave, onCancel }: { transaction: StockTransaction; onSave: () => void; onCancel: () => void }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await axios.put(`${API_ENDPOINTS.stockMove}/${transaction.id}`, {
        partType: form.partType,
        tanggal: form.tanggal,
        namaBarang: form.namaBarang.trim(),
        typeBarang: form.typeBarang.trim(),
      ...
      const errorMessage = handleError(err, 'Failed to update transaction')
      Swal.fire({
        ...SWAL_ERROR_CONFIG,
        title: 'Gagal!',
        text: errorMessage || ERROR_MESSAGES.updateFailed,
      })
    } finally {
      setSaving(false)
    }
