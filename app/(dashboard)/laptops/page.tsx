...
export default function LaptopsPage() {
  ...
  const {
    ...
    entityName: 'Laptop',
    entityNamePlural: 'Laptop',
    colorTheme: 'blue',
  })

  // Handler Buat Serah Terima — hanya aktif saat 1 item dipilih
  const handleSerahTerima = useCallback(async () => {
    if (selectedItems.size !== 1) return
    const laptopId = Array.from(selectedItems)[0]

    const { value: nomorSurat } = await Swal.fire({
      title: 'Nomor Surat',
      input: 'text',
      inputLabel: 'Masukkan nomor surat (contoh: 148)',
      inputPlaceholder: '148',
    ...
    if (!nomorSurat && nomorSurat !== '') return
    if (nomorSurat === undefined) return

    try {
      const response = await axios.post(
        '/api/laptops/serah-terima',
        { id: laptopId, nomorSurat },
        { responseType: 'blob' }
      )
      ...
      a.download = match ? match[1] : 'serah-terima.docx'
      a.href = url
      a.click()
      ...
      Swal.fire({
        title: 'Gagal!',
        text: 'Gagal membuat dokumen serah terima.',
        icon: 'error',
  ...
  const { handleImport } = useExcelImport({    apiEndpoint: '/api/laptops',
    entityName: 'Laptop',
    entityNamePlural: 'Laptop',
    importConfig: {
      headerMap: LAPTOP_HEADER_MAP,
  ...
  const { handleExport } = useExcelExport({
    ...
    columns: [
      ...
      { header: 'MERK', key: 'merk', width: 30 },
      { header: 'PROSESOR', key: 'prosesor', width: 30 },
      { header: 'STORAGE', key: 'ssdHdd', width: 15 },
      ...
      { header: 'TANGGAL MASUK BARANG', key: 'masuk', width: 18 },
      { header: 'TANGGAL KIRIM BARANG', key: 'kirim', width: 18 },
      { header: 'JUMLAH UNIT', key: 'unit', width: 15 },
      { header: 'DIPERUNTUKKAN KEPADA', key: 'untuk', width: 25 },
      ...
      { header: 'FOTO GAMBAR', key: 'gambar', width: 30 },
    ],
    filename: 'Data_Laptop',
    ...
    <div className="min-h-screen mesh-gradient dark:mesh-gradient-dark p-4 sm:p-6 lg:p-8 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs sm:text-sm text-gray-400 font-bold uppercase tracking-[0.2em] opacity-60">Inventory / Computing Units</p>
        </div>
          ...
          <div className="p-4 sm:p-6 lg:p-8 border-b border-primary/20 bg-primary/5/50 dark:bg-primary/50/5 backdrop-blur-sm">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {/* Buat Serah Terima — hanya muncul saat tepat 1 item dipilih */}
                {selectedItems.size === 1 && canSerahTerima && (
                  <button
                    onClick={handleSerahTerima}
                    className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-500/20 hover:scale-105 active:scale-95"
                  >
                ...
                <tr>
                  <td colSpan={10} className="px-4 sm:px-8 py-24 sm:py-32 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-50">
                      ...
                      <span className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">No Laptop Registered</span>
                    </div>
                  </td>
                </tr>
              ) : (
                laptops.map((laptop: Laptop, index: number) => {
                  const rowNumber = (currentPage - 1) * itemsPerPage + index + 1
                  return (
                    <tr key={laptop.id} className={`group hover:bg-primary/5/30 dark:hover:bg-primary/50/[0.02] transition-all duration-500 ease-out ${selectedItems.has(laptop.id) ? 'bg-primary/5/50 dark:bg-primary/50/10' : ''}`}>
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                        <div className="flex items-center">
                          <input
                            type="checkbox"
                            checked={selectedItems.has(laptop.id)}
                            onChange={(e) => handleSelectItem(laptop.id, e.target.checked)}
                            className="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-primary focus:ring-2 cursor-pointer"
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                        <Link href={`/laptops/${laptop.id}/view`} className="flex flex-col hover:opacity-80 transition-opacity cursor-pointer">
                          <span className="text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight group-hover:text-primary transition-colors">{laptop.merk}</span>
                          {laptop.sn && (
                            <span className="text-[9px] sm:text-[10px] text-primary font-black uppercase tracking-widest mt-0.5">SN: {laptop.sn}</span>
                          )}
                        </Link>
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                        <div className="flex flex-col">
                          <span className="text-[10px] sm:text-xs font-black text-gray-700 dark:text-gray-300 uppercase tracking-tight">{laptop.prosesor}</span>
                          <span className="text-[9px] sm:text-[10px] text-primary font-black uppercase tracking-widest mt-0.5">{laptop.ram} RAM / {laptop.ssdHdd}</span>
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                        <span className="text-xs sm:text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight underline decoration-primary/20 underline-offset-8 decoration-2">{laptop.untuk}</span>
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                      ...
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                        <div className={`inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-1.5 rounded-full ring-1 ring-inset ${
                          laptop.status?.toUpperCase() === 'BARU'
                            ? 'bg-green-500/5 text-green-500 ring-green-500/20'
                            : laptop.status?.toUpperCase() === 'SECOND'
                            ? 'bg-yellow-500/5 text-yellow-500 ring-yellow-500/20'
                            : laptop.status?.toUpperCase() === 'SERVICE'
                            ? 'bg-red-500/5 text-red-500 ring-red-500/20'
                            : laptop.status?.toLowerCase().includes('rusak') || laptop.status?.toLowerCase().includes('error')
                            ? 'bg-red-500/5 text-red-500 ring-red-500/20'
                          ...
                          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] whitespace-nowrap">{laptop.status}</span>
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                        {laptop.gambar ? (
                          <div className="flex items-center justify-center">
                            {imageErrors.has(laptop.id) ? (
                              ...
                              <Image
                                ...
                                unoptimized={laptop.gambar?.startsWith('/')}
                                onError={() => {
                                  setImageErrors(prev => new Set(prev).add(laptop.id))
                                }}
                              />
                            )}
                          </div>
                      ...
                      <td className="px-3 sm:px-4 py-2.5 sm:py-3">
                        <div className="flex items-center justify-end gap-1 sm:gap-2 lg:gap-3">
                          {/* PDF Serah Terima Button */}
                          {laptop.serahTerimaPdf ? (
                            <a
                              href={laptop.serahTerimaPdf}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-2.5 bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-white/5 rounded-xl text-red-600 hover:text-red-700 hover:scale-110 hover:shadow-xl hover:shadow-red-500/10 transition-all"
                              title="Lihat PDF Serah Terima"
                            >
                              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z"/><path d="M14 2v6h6"/><path d="M9 13h6M9 17h6"/></svg>
                            </a>
                          ) : (
                            <button
                              disabled
                              className="p-2.5 bg-white dark:bg-[#0f172a] border border-gray-100 dark:border-white/5 rounded-xl text-gray-300 cursor-not-allowed opacity-50"
                              title="Belum ada PDF"
                            >
