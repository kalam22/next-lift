...
export default function UPSPage() {
  ...
  const { handleExport } = useExcelExport({
    ...
    columns: [
      ...
      { header: 'DAYA(VA)', key: 'dayaVa', width: 15 },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl sm:text-4xl font-black text-[#020617] dark:text-white tracking-tighter uppercase">
            Data <span className="text-primary">UPS</span>
          </h1>
          <p className="text-xs sm:text-sm text-gray-400 font-bold uppercase tracking-[0.2em] opacity-60">Inventory / UPS Units</p>
        </div>
                ...
                <tr>
                  <td colSpan={11} className="px-4 sm:px-8 py-24 sm:py-32 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-50">
                      ...
                      <span className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">No UPS Registered</span>
                    </div>
                  </td>
                </tr>
              ) : (
                UPSs.map((UPS: Ups, index: number) => {
                  const rowNumber = (currentPage - 1) * itemsPerPage + index + 1
                  return (
                    <tr key={UPS.id} className={`group hover:bg-primary/5/30 dark:hover:bg-primary/50/[0.02] transition-all duration-500 ease-out ${selectedItems.has(UPS.id) ? 'bg-primary/5/50 dark:bg-primary/50/10' : ''}`}>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <div className="flex items-center">
                          <input
                            type="checkbox"
                            checked={selectedItems.has(UPS.id)}
                            onChange={(e) => handleSelectItem(UPS.id, e.target.checked)}
                            className="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-primary focus:ring-2 cursor-pointer"
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <Link href={`/ups/${UPS.id}/view`}>
                          <span className="text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight group-hover:text-primary transition-colors cursor-pointer hover:text-primary">{UPS.brand}</span>
                        </Link>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs font-black text-gray-700 dark:text-gray-300">{UPS.jumlahOrderan}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs sm:text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight underline decoration-primary/20 underline-offset-8 decoration-2">{UPS.diperuntukan}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                      ...
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs font-bold text-gray-600 dark:text-gray-300">#{UPS.nomorPO}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <div className={`inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-1.5 rounded-full ring-1 ring-inset ${
                          ...
                          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] whitespace-nowrap">{UPS.statusBarang}</span>
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        {UPS.foto ? (
                          <div className="flex items-center justify-center">
                            {imageErrors.has(UPS.id) ? (
                              ...
                              <Image
                                ...
                                onError={() => {
                                  setImageErrors(prev => new Set(prev).add(UPS.id))
                                }}
                              />
                            )}
                          </div>
