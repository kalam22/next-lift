...
export default function ViewUPS() {
    ...
    const fetchUPS = useCallback(async () => {
        try {
            const [upsRes, navRes] = await Promise.all([
                axios.get<Ups>(`/api/ups/${params.id}`),
                axios.get<NavInfo>(`/api/ups/navigation?id=${params.id}`),
            ])
            setUps(upsRes.data)
            setNav(navRes.data)
        } catch (error: unknown) {
        ...
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div className="flex items-center justify-center sm:justify-start gap-5">
                    <Link href="/ups" className="p-2.5 text-gray-400 hover:text-primary hover:bg-white dark:hover:bg-[#1e293b] rounded-2xl border border-transparent hover:border-[#f1f5f9] dark:hover:border-[#334155] transition-all shadow-sm">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                    </Link>
                    <div>
                        <h1 className="text-3xl font-black text-[#0f172a] dark:text-white tracking-tighter uppercase">{ups.brand}</h1>
                        <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs font-bold text-primary uppercase tracking-widest">{ups.diperuntukan}</span>
                            <span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span>
                            <span className="text-xs font-medium text-gray-400">Order: {ups.jumlahOrderan} Unit</span>
                            {nav.total > 0 && (
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1">
                        <button
                            onClick={() => nav.prevId && router.push(`/ups/${nav.prevId}/view`)}
                            disabled={!nav.prevId}
                        ...
                        <button
                            onClick={() => nav.nextId && router.push(`/ups/${nav.nextId}/view`)}
                            disabled={!nav.nextId}
                    ...
                    <Link href={`/ups/${ups.id}/edit`} className="px-6 py-2.5 bg-white dark:bg-[#1e293b] border border-[#f1f5f9] dark:border-[#334155] rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-600 dark:text-gray-300 hover:bg-gray-50 transition-all shadow-sm">
                        Edit Perangkat
                    </Link>
                </div>
            </div>
                ...
                <div className="lg:col-span-4 space-y-8">
                        ...
                        <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-50 dark:bg-[#0f172a] group">
                            {ups.foto ? (
                                <Image 
                                    ...
                                    unoptimized={ups.foto?.startsWith('/')}
                                    onError={(e) => {
                    ...
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-8 space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Site Lokasi</span><span className="text-xs font-black text-primary dark:text-primary uppercase tracking-widest">{ups.site}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Departemen</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{ups.departemen || '-'}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Nomor PO</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">#{ups.nomorPO}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Tgl Masuk</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{ups.tanggalMasuk ? formatDateWITA(ups.tanggalMasuk) : '-'}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Tgl Kirim</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{ups.tanggalKirim ? formatDateWITA(ups.tanggalKirim) : '-'}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Surat Jalan</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{ups.nomorSuratJalan || '-'}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 space-y-8">
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-[#f1f5f9] dark:border-[#334155] bg-gray-50/50 dark:bg-[#0f172a]/20">
                            <h3 className="text-[10px] font-black text-[#0f172a] dark:text-white uppercase tracking-[0.2em]">Informasi UPS</h3>
                        </div>
                        <div className="p-8 grid grid-cols-2 sm:grid-cols-3 gap-6">
                            <SpecItem label="Brand" value={ups.brand} />
                            <SpecItem label="Daya (VA)" value={ups.dayaVa || '-'} />
                            <SpecItem label="Jumlah Orderan" value={ups.jumlahOrderan?.toString() || '-'} />
                            <SpecItem label="Status Barang" value={ups.statusBarang} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-8 h-full">
                            ...
                            <p className="text-sm font-medium text-gray-400 dark:text-gray-400 leading-relaxed">
                                {ups.keterangan || 'Belum ada keterangan untuk unit ini.'}
                            </p>
                        </div>
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <div className="space-y-2">
                                        {ups.diperuntukan ? (
                                            ups.diperuntukan.split(',').map((item: string, index: number) => (
                                ...
                                <div className="pt-3 border-t border-[#f1f5f9] dark:border-[#334155]">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-xs font-bold text-gray-600 dark:text-gray-300">{ups.jumlahOrderan} Unit</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Metadata */}
                    <MetadataCard createdAt={ups.createdAt} updatedAt={ups.updatedAt} />
                </div>
            </div>
        </div>
    )
