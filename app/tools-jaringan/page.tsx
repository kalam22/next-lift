...
export default function ToolsJaringanPage() {
  ...
  const {
    ...
    fetchData,
    colorTheme,
  } = useDataTable<ToolsJaringan>({
    apiEndpoint: '/api/tools-jaringan',
    entityName: 'ToolsJaringan',
    entityNamePlural: 'Tools Jaringan',
    colorTheme: 'blue',
  })
  ...
  const { handleImport } = useExcelImport({
    ...
    importConfig: {
      ...
      requiredFields: ['brand', 'jumlahOrderan', 'diperuntukan', 'site', 'nomorPO', 'statusBarang', 'tanggalMasuk'],
    },
    onSuccess: () => {
      fetchData()
    },
  ...
  const { handleExport } = useExcelExport({
    ...
    columns: [
      ...
      { header: 'MERK', key: 'brand', width: 30 },
    ...
    filename: 'Data_Tools_Jaringan',
    worksheetName: 'Data Tools Jaringan',
    imageField: {
      fieldName: 'foto',
      columnIndex: 12,
    },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl sm:text-4xl font-black text-[#020617] dark:text-white tracking-tighter uppercase">
            Data <span className="text-primary">Tools Jaringan</span>
          </h1>
          <p className="text-xs sm:text-sm text-gray-400 font-bold uppercase tracking-[0.2em] opacity-60">Inventory / Tools Jaringan Units</p>
        </div>
        <div className="flex items-center justify-center sm:justify-start gap-3 flex-wrap">
          ...
          >
            ...
            <span className="uppercase tracking-widest">Register Tools Jaringan</span>
          </Link>
        </div>
      </div>
        ...
        <div className="p-4 sm:p-6 lg:p-8 border-b border-[#f1f5f9]/50 dark:border-[#1e293b]/50 bg-white/50 dark:bg-black/20 backdrop-blur-3xl">
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            <div className="relative w-full lg:w-auto lg:flex-1 group">
              ...
              <input
                type="text"
                placeholder="Cari brand, diperuntukan, atau site..."
                value={searchTerm}
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-gray-50/30 dark:bg-black/10 uppercase font-black text-[10px] tracking-[0.25em] text-gray-400/80">
                ...
                <th className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6 min-w-[80px] text-center">NO</th>
                <th className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6 min-w-[150px]">BRAND</th>
                <th className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6 min-w-[120px]">JUMLAH ORDERAN</th>
                ...
                <th className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6 min-w-[120px]">NOMOR PO</th>
            ...
            <tbody className="divide-y divide-gray-100/50 dark:divide-white/[0.02]">
              {loading ? (
                <tr>
                  <td colSpan={11} className="px-4 sm:px-8 py-24 sm:py-32 text-center">
                ...
                <tr>
                  <td colSpan={11} className="px-4 sm:px-8 py-24 sm:py-32 text-center">
                    <div className="flex flex-col items-center gap-4 opacity-50">
                      ...
                      <span className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">No Tools Jaringan Registered</span>
                    </div>
                  </td>
                </tr>
              ) : (
                toolsJaringans.map((toolsJaringan: ToolsJaringan, index: number) => {
                  const rowNumber = (currentPage - 1) * itemsPerPage + index + 1
                  return (
                    <tr key={toolsJaringan.id} className={`group hover:bg-primary/5/30 dark:hover:bg-primary/50/[0.02] transition-all duration-500 ease-out ${selectedItems.has(toolsJaringan.id) ? 'bg-primary/5/50 dark:bg-primary/50/10' : ''}`}>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <div className="flex items-center">
                          <input
                            type="checkbox"
                            checked={selectedItems.has(toolsJaringan.id)}
                            onChange={(e) => handleSelectItem(toolsJaringan.id, e.target.checked)}
                            className="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-primary focus:ring-2 cursor-pointer"
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <Link href={`/tools-jaringan/${toolsJaringan.id}/view`}>
                          <span className="text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight group-hover:text-primary transition-colors cursor-pointer hover:text-primary">{toolsJaringan.brand}</span>
                        </Link>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs font-black text-gray-700 dark:text-gray-300">{toolsJaringan.jumlahOrderan}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs sm:text-sm font-black text-[#020617] dark:text-white uppercase tracking-tight underline decoration-primary/20 underline-offset-8 decoration-2">{toolsJaringan.diperuntukan}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                      ...
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="inline-flex items-center px-3 sm:px-4 py-1.5 rounded-xl bg-gray-100 dark:bg-white/5 border border-white/50 dark:border-white/5 text-[9px] sm:text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest shadow-sm whitespace-nowrap">
                          {toolsJaringan.departemen || '-'}
                        </span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <span className="text-xs font-bold text-gray-600 dark:text-gray-300">#{toolsJaringan.nomorPO}</span>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <div className={`inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-1.5 rounded-full ring-1 ring-inset ${
                          ...
                          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] whitespace-nowrap">{toolsJaringan.statusBarang}</span>
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        {toolsJaringan.foto ? (
                          <div className="flex items-center justify-center">
                            {imageErrors.has(toolsJaringan.id) ? (
                              ...
                              <Image
                                ...
                                onClick={() => {
                                  if (toolsJaringan.foto) {
                                    window.open(toolsJaringan.foto, '_blank')
                                  }
                                }}
                                loading="lazy"
                                placeholder="blur"
                                ...
                                onError={() => {
                                  setImageErrors(prev => new Set(prev).add(toolsJaringan.id))
                                }}
                              />
                            )}
                          </div>
                      ...
                      <td className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
                        <div className="flex items-center justify-end gap-1 sm:gap-2 lg:gap-3">
                          <Link
                            href={`/tools-jaringan/${toolsJaringan.id}/view`}
                          ...
                          <Link
                            href={`/tools-jaringan/${toolsJaringan.id}/edit`}
                          ...
                          <button
                            onClick={() => handleDelete(toolsJaringan.id)}
