...
export default function ViewToolsJaringan() {
    ...
    const fetchToolsJaringan = async () => {
        try {
            const response = await axios.get<ToolsJaringan>(`/api/tools-jaringan/${params.id}`)
            setToolsJaringan(response.data)
        } catch (error: unknown) {
            logger.error('Error fetching tools jaringan:', error)
            await Swal.fire({
                title: 'Gagal!',
                text: 'Gagal memuat data Tools Jaringan',
                icon: 'error',
        ...
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div className="flex items-center justify-center sm:justify-start gap-5">
                    <Link href="/tools-jaringan" className="p-2.5 text-gray-400 hover:text-primary hover:bg-white dark:hover:bg-[#1e293b] rounded-2xl border border-transparent hover:border-[#f1f5f9] dark:hover:border-[#334155] transition-all shadow-sm">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                    </Link>
                    <div>
                        <h1 className="text-3xl font-black text-[#0f172a] dark:text-white tracking-tighter uppercase">{toolsJaringan.brand}</h1>
                        <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs font-bold text-primary uppercase tracking-widest">{toolsJaringan.diperuntukan}</span>
                            <span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span>
                            <span className="text-xs font-medium text-gray-400">Order: {toolsJaringan.jumlahOrderan} Unit</span>
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                        toolsJaringan.statusBarang?.toUpperCase() === 'BARU'
                            ? 'bg-green-50 dark:bg-green-500/10 text-green-600 border-green-100 dark:border-green-900/40'
                            : toolsJaringan.statusBarang?.toUpperCase() === 'SECOND'
                            ? 'bg-yellow-50 dark:bg-yellow-500/10 text-yellow-600 border-yellow-100 dark:border-yellow-900/40'
                            : toolsJaringan.statusBarang?.toUpperCase() === 'SERVICE'
                        ...
                        {toolsJaringan.statusBarang || '-'}
                    </span>
                    <Link href={`/tools-jaringan/${toolsJaringan.id}/edit`} className="px-6 py-2.5 bg-white dark:bg-[#1e293b] border border-[#f1f5f9] dark:border-[#334155] rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-600 dark:text-gray-300 hover:bg-gray-50 transition-all shadow-sm">
                        Edit Perangkat
                    </Link>
                </div>
            </div>
                ...
                <div className="lg:col-span-4 space-y-8">
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm overflow-hidden p-3">
                        <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-50 dark:bg-[#0f172a] group">
                            {toolsJaringan.foto ? (
                                <Image 
                                    src={toolsJaringan.foto} 
                                    alt={toolsJaringan.brand} 
                                    fill 
                                    ...
                                    unoptimized={toolsJaringan.foto?.startsWith('/')}
                                    onError={(e) => {
                    ...
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-8 space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Site Lokasi</span><span className="text-xs font-black text-primary dark:text-primary uppercase tracking-widest">{toolsJaringan.site}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Departemen</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{toolsJaringan.departemen || '-'}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Nomor PO</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">#{toolsJaringan.nomorPO}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Tgl Masuk</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{toolsJaringan.tanggalMasuk ? formatDateWITA(toolsJaringan.tanggalMasuk) : '-'}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Tgl Kirim</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{toolsJaringan.tanggalKirim ? formatDateWITA(toolsJaringan.tanggalKirim) : '-'}</span></div>
                            <div className="flex justify-between items-center"><span className="text-[11px] font-bold text-gray-400 uppercase">Surat Jalan</span><span className="text-xs font-bold text-gray-600 dark:text-gray-300">{toolsJaringan.nomorSuratJalan || '-'}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 space-y-8">
                    <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm overflow-hidden">
                        ...
                        <div className="p-8 grid grid-cols-2 sm:grid-cols-3 gap-6">
                            ...
                            <SpecItem label="Status Barang" value={toolsJaringan.statusBarang || '-'} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-[#f1f5f9] dark:border-[#334155] shadow-sm p-8 h-full">
                            ...
                            <p className="text-sm font-medium text-gray-400 dark:text-gray-400 leading-relaxed">
                                {toolsJaringan.keterangan || 'Belum ada keterangan untuk unit ini.'}
                            </p>
                        </div>
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <div className="space-y-2">
                                        {toolsJaringan.diperuntukan ? (
                                            toolsJaringan.diperuntukan.split(',').map((item: string, index: number) => (
                                ...
                                <div className="pt-3 border-t border-[#f1f5f9] dark:border-[#334155]">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-xs font-bold text-gray-600 dark:text-gray-300">{toolsJaringan.jumlahOrderan} Unit</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Metadata */}
                    <MetadataCard createdAt={toolsJaringan.createdAt} updatedAt={toolsJaringan.updatedAt} />
                </div>
            </div>
        </div>
    )
