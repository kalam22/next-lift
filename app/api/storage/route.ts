...
import type { Storage, ApiResponse } from '@/types/entities'
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Generate cache key
    const cacheKey = cache.generateKey('/api/storage', {
      page: page.toString(),
      limit: limitParam,
      search: search || null,
      sort_by: sortByParam,
    ...
    // Check cache
    const cachedResponse = await cache.get<ApiResponse<Storage>>(cacheKey)
    if (cachedResponse) {
    ...
    const queryPromise = prisma.storage.findMany({
      where,
      select: {
        id: true,
        brand: true,
    ...
    const [storages, total] = await Promise.all([
      queryPromise,
      needsTotal ? prisma.storage.count({ where }) : Promise.resolve(0),
    ])

    const responseData: ApiResponse<Storage> = {
      data: storages as Storage[],
    ...
    return NextResponse.json(
      { error: 'Failed to fetch storage', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    logActivity({ entityType: 'storage', entityId: storage.id, action: 'CREATE', description: `Data Storage "${validatedData.brand}" ditambahkan`, userId: user?.id, userName: user?.name })

    return successResponse(storage, 201)
  } catch (error: unknown) {
