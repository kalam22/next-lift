...
// GET /api/laptops/navigation?id=123
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all IDs ordered by createdAt desc (same order as the list page)
    const all = await prisma.laptops.findMany({
      select: { id: true },
    ...
    return handleDbError(error, 'mengambil navigasi laptop')
  }
}
